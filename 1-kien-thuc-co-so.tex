...
Xét phương trình đạo hàm cấp 1 của hàm trùng phương $y = ax^4 + bx^2 + c \quad (a \neq 0)$:
...
\begin{itemize}
    \item Vì chỉ chứa các số mũ chẵn của biến $x$ nên \textbf{hàm trùng phương là một hàm số chẵn và có đồ thị đối xứng qua trục tung}. Thật vậy,
    \[
    f(-x)
    =a(-x)^4+b(-x)^2+c
    =ax^4+bx^2+c
 ...
 \begin{figure}[H]
    \centering
    % Đồ thị bên trái: 1 điểm cực trị
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \begin{tikzpicture}[scale=0.75]
            \begin{axis}[
                title = {\small $a\cdot b \ge 0$ (1 cực trị)},
                axis lines = middle,
                xlabel = {$x$}, ylabel = {$y$},
                xmin = -1.8, xmax = 1.8,
                ymin = -0.5, ymax = 3.5,
                ...
                \addplot [domain=-1.3:1.3, samples=100, smooth, thick, red] {x^4 + 2*x^2 + 1};
                % Đường nét đứt minh họa đối xứng
                \draw[dashed, gray] (axis cs:-1,3) -- (axis cs:1,3);
                \filldraw[blue] (axis cs:-1,3) circle (1.5pt);
                \filldraw[blue] (axis cs:1,3) circle (1.5pt);
            \end{axis}
        \end{tikzpicture}
    \end{minipage}
    \hfill
    % Đồ thị bên phải: 3 điểm cực trị
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \begin{tikzpicture}[scale=0.75]
            \begin{axis}[
                title = {\small $a\cdot b < 0$ (3 cực trị)},
                axis lines = middle,
                xlabel = {$x$}, ylabel = {$y$},
                xmin = -1.8, xmax = 1.8,
                ymin = -0.5, ymax = 2.5,
                ...
                \addplot [domain=-1.6:1.6, samples=100, smooth, thick, blue] {x^4 - 2*x^2 + 1};
                % Đường nét đứt minh họa đối xứng 2 đáy cực tiểu
                \draw[dashed, gray] (axis cs:-1,0) -- (axis cs:1,0);
                \filldraw[red] (axis cs:-1,0) circle (1.5pt);
                \filldraw[red] (axis cs:1,0) circle (1.5pt);
            \end{axis}
        \end{tikzpicture}
    \end{minipage}
    \caption{Đồ thị hàm trùng phương luôn đối xứng qua trục tung $Oy$}
\end{figure}
    \item\textbf{ Tập xác định của hàm số là}
    \[
    D=\mathbb{R}.
    \]

    \item \textbf{ Đồ thị hàm trùng phương có thể có tối đa ba điểm cực trị}, gồm:
    \begin{itemize}
        \item hai điểm cực tiểu và một điểm cực đại;
        \item hoặc hai điểm cực đại và một điểm cực tiểu.
    \end{itemize}

    \item \textbf{Tính đối xứng của các điểm cực trị (Trường hợp $ab < 0$):} \\
    Khi $ab < 0$, phương trình đạo hàm $y' = 4ax^3 + 2bx = 2x(2ax^2 + b) = 0$ có 3 nghiệm phân biệt. 
    Ngoài nghiệm $x_0 = 0$ (ứng với điểm cực trị nằm trên trục tung $Oy$), hai điểm cực trị còn lại có hoành độ là nghiệm của phương trình $2ax^2 + b = 0 \iff x^2 = -\dfrac{b}{2a}$. 
    
    Do $a$ và $b$ trái dấu ($ab < 0$) nên $-\dfrac{b}{2a} > 0$, từ đó ta thu được hai hoành độ đối nhau qua gốc tọa độ $O$:
    \[
    x_1 = -\sqrt{-\frac{b}{2a}},
    \qquad
    x_2 = \sqrt{-\frac{b}{2a}}.
    \]
    Chính nhờ hai hoành độ này đối nhau ($x_1 = -x_2$) kết hợp với tính chất hàm số chẵn $f(x_1) = f(x_2)$, hai điểm cực trị này luôn nằm ở vị trí đối xứng hoàn toàn qua trục tung $Oy$, làm cho tam giác tạo bởi ba điểm cực trị luôn là một tam giác cân tại đỉnh $A(0; c)$.
\end{itemize}
